...
function Graph(props) {
  ...
  function renderDag() {
    ...
    let render = new dagreD3.render(); // Set up an SVG group so that we can translate the final graph.
    ...
    render(svgGroup, g); // ================= label beside node ==================

    let nd = svgGroup.append('g').attr('class', 'labels');
    g.nodes().forEach((v, i) => {
      let node = g.node(i);
      nd.append('text').attr('text-anchor', 'middle').attr('id', node.label).attr('x', node.x).attr('y', -30 + node.y).text(node.name);
      nd.append('g').attr('id', 'node' + i).attr('transform', 'translate(' + node.x + ' ' + node.y + ')').append('path');
    }); // ================= arc for node ==================

    let time_offset = 0;
    let arcGenerator = d3.arc().innerRadius(20).outerRadius(23).startAngle(0);
    let increase = true;
    let shallowp = d3.rgb(184, 187, 222),
        deepp = d3.rgb(128, 0, 128);
    ...
    d3.interval(() => {
      ...
      g.nodes().forEach((v, i) => {
        ...
        if (typeof event_result[id2event[i]] !== 'undefined' && event_result[id2event[i]][1].final_result === 'Happen') {
          nd.select('#node' + i).select('path').datum({
            endAngle: time_offset / 180 * Math.PI
          }).style('fill', compute(time_offset / 360)).attr('d', arcGenerator);
        }
      });
    }, 100); // ================= text on edge path ==================  
    ...
    pp.each(function (p, i) {
      ...
      d3.select(this).append('text').attr('text-anchor', 'middle').append('textPath').attr('href', '#path' + i).attr('startOffset', '50%').text(edge_labels[i]);
    }); // ================= tooltip ==================    
    // for nodes        

    svgGroup.selectAll('g.node').on('mouseover', function (v, i) {
      let tt = d3.select("#tooltip").attr("style", "left:" + v.pageX + "px" + "; top:" + v.pageY + "px").select("#tooltip_value").append('span');
    ...
    svgGroup.selectAll("g.node").on('mouseout', function (v, i) {
      ...
      let tt = d3.select("#tooltip").attr("style", "left:" + v.pageX + "px" + "; top:" + v.pageY + "px").select("#tooltip_value");
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("svg", {
    ...
    align: "center"
  }, /*#__PURE__*/React.createElement("strong", null, "Tooltip:")), /*#__PURE__*/React.createElement("p", {
    id: "tooltip_value"
  })));
}
